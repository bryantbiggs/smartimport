...
def authenticate_github(token_file):
...
def show_rate_remaining(git_client):
	...
	'''
	print('Resource rate limit remaining: {0}'.format(get_rate_remaining(git_client, 'core')))
	print('Search rate limit remaining: {0}'.format(get_rate_remaining(git_client, 'search')))

def check_rate_limit(location, ct=0):
	'''
	IN: 
...
def get_repos_of_top_repos(ct=0, new_only=True):
